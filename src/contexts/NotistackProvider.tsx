import { ReactNode, useRef } from "react";
import { SnackbarProvider, MaterialDesignContent } from "notistack";
// @mui
import { styled, alpha, useTheme } from "@mui/material/styles";
import { Box, GlobalStyles, Button } from "@mui/material";

//
import Iconify from "@/app/components/Iconify";
// import { IconButtonAnimate } from './animate';

// ----------------------------------------------------------------------

type NotistackProviderPayload = {
  children: ReactNode;
};

export default function NotistackProvider({
  children,
}: NotistackProviderPayload) {
  const theme: any = useTheme();
  console.log("theme.palette.mode:", theme.palette.mode);
  const isLight = theme.palette.mode === "light";
  const notistackRef: any = useRef(null);

  const onClose = (key: any) => () => {
    notistackRef.current.closeSnackbar(key);
  };

  const StyledMaterialDesignContent = styled(MaterialDesignContent)(() => ({
    "&.notistack-MuiContent-success, &.notistack-MuiContent-error": {
      width: "100%",
      padding: theme.spacing(1),
      margin: theme.spacing(0.25, 0),
      boxShadow: theme.customShadows.z8,
      borderRadius: theme.shape.borderRadius,
      color: (theme.palette.grey as any)[isLight ? 800 : 0],
      backgroundColor: (theme.palette.grey as any)[isLight ? 0 : 900],
    },
  }));

  return (
    <>
      <SnackbarProvider
        ref={notistackRef}
        dense
        maxSnack={5}
        preventDuplicate
        autoHideDuration={3000}
        variant="success" // Set default variant
        anchorOrigin={{ vertical: "top", horizontal: "right" }}
        iconVariant={{
          info: <SnackbarIcon icon={"eva:info-fill"} color="info" />,
          success: (
            <SnackbarIcon
              icon={"eva:checkmark-circle-2-fill"}
              color="success"
            />
          ),
          warning: (
            <SnackbarIcon icon={"eva:alert-triangle-fill"} color="warning" />
          ),
          error: <SnackbarIcon icon={"eva:alert-circle-fill"} color="error" />,
        }}
        // With close as default
        action={(key) => (
          // <IconButtonAnimate size="small" onClick={onClose(key)} sx={{ p: 0.5 }}>
          //   <Iconify icon={'eva:close-fill'} />
          // </IconButtonAnimate>
          <Button size="small" onClick={onClose(key)} sx={{ p: 0.5 }}>
            <Iconify icon={"eva:close-fill"} />
          </Button>
        )}
        Components={{
          success: StyledMaterialDesignContent,
          error: StyledMaterialDesignContent,
        }}
      >
        {children}
      </SnackbarProvider>
    </>
  );
}

// ----------------------------------------------------------------------

type SnackbarIconPayload = {
  icon: string;
  color: "primary" | "secondary" | "info" | "success" | "warning" | "error";
};

function SnackbarIcon({ icon, color }: SnackbarIconPayload) {
  return (
    <Box
      component="span"
      sx={{
        mr: 1.5,
        width: 40,
        height: 40,
        display: "flex",
        borderRadius: 1.5,
        alignItems: "center",
        justifyContent: "center",
        color: `${color}.main`,
        bgcolor: (theme) => alpha(theme.palette[color].main, 0.16),
      }}
    >
      <Iconify icon={icon} width={24} height={24} />
    </Box>
  );
}
