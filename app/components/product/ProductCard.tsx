...
function ProductCard({ data, className = "" }: ProductCardProps) {
  ...
  return (
    <div
      className={`w-full max-w-[250px] bg-white rounded-md shadow-md p-0 ${className}`}
    >
      ...
      <div className="px-2 pt-1 pb-2">
        <h3 className="text-sm m-0">{data.name}</h3>
        <div className="flex flex-row justify-between">
          <div>
            <Rating size="small" value={4} precision={0.1} readOnly />
            <div className="flex flex-row">
              {data?.priceSale > 0 && (
                <p className="m-0 text-black-400 line-through">
                  {fCurrency(data?.price)}
                </p>
              )}
              <p className=" ml-2 my-0">{fCurrency(price)}</p>
            </div>
          </div>
